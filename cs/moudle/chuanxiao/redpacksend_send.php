...
function sendPack($obj)
{
    ...
    */
    ...
    {
        $send_name="￥".$res[$i]['fenchengjine']." ";
        $re_openid =$res[$i]['send_openid'];
        $total_amount=$res[$i]['fenchengjine']*100;
        $total_num=1;
        $wishing="日期：".$res[$i]['add_time']."，金额：￥".($res[$i]['fenchengjine'])."，收款快乐!";
        $act_name="发送红包";
        $remark = "日期：".$res[$i]['add_time']."，金额：￥".($res[$i]['fenchengjine'])."，收款快乐!";
        
        
        $tools = new JsApiPay();
        $sendRedPa = $tools->GetSendRedPack_arr($send_name,$re_openid,$total_amount,$total_num,$wishing,$act_name,$remark);
        
    
        $retrun = WxPayApi::sendRedPack($sendRedPa);
        print_r($retrun);
...
//    [return_msg] => 发放成功
//    [result_code] => SUCCESS
        if($retrun['return_code']=='SUCCESS' and $retrun['return_msg']=='发放成功' and $retrun['result_code']=='SUCCESS')
        {
            $field_values = array("is_send" => 1,"send_time"=>time(),"send_err"=>$retrun['return_msg']);
