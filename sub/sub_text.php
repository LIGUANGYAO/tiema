...
function text($Nu, $tb)
{
    ...
    if (isset($_REQUEST['m_key'])) {
        $filer1 = " and ( text_sn like '%" . trim($_REQUEST['m_key']) .
            "%' or text like '%" . trim($_REQUEST['m_key']) . "%' or title like '%" . trim($_REQUEST['m_key']) . "%')";
    } else {
        $filer1 = '';
    }
...
function i_text($tb, $field, $time_f)
{
...
{   
      $sql="select id,
            ...
            last_update,
            last_update_2 
            from text_reply where text_sn='".$obj."' ";
    $res = $GLOBALS['db']->getAll($sql);
    //$noNum=20;
...
function update_text_mx($tb,$field,$u_id,$time_f)
{   
