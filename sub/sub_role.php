...
function get_role_list($Nu, $ma, $sql)
{
    ...
    if (isset($_REQUEST['Action'])) {
     ...
     if (isset($_REQUEST['m_key'])) {
        $filer1 = " and (role_sn like '%" . trim($_REQUEST['m_key']) . "%'  or role_name like '%" . trim($_REQUEST['m_key']) . "%' ) ";
    } else {
        $filer1 = '';
    }
...
{   
    ...
    //$sql = "select role_sn,role_name,role_outer_name,role_note_1,role_note_2,role_note_3,role_note_4,role_note_5,role_name_eg,zz,tzsy,action_url,last_update,last_update_2,start_time,end_time,sort_no  from role " 
    $sql=$sql.$where . "'";
...
function update_role_mx($tb,$field,$u_id)
{   
     //$tb,$field,$u_id表，字段，关键字
    //print_r($field);
   // $field="role_name,role_note_1,role_note_2";
    $array = explode(",",$field);
...
//    {
//        $get_u_info="select user_id,user_code,user_name from admin_user where user_id='".$select2[$i]."'";
//        $get_u_info = $GLOBALS['db']->getRow($get_u_info);
//        $insert="insert into role_user(p_id,u_id,user_sn,user_name)  values ('".$_REQUEST['role_sn']."','".$get_u_info['user_id']."','".$get_u_info['user_code']."','".$get_u_info['user_name']."')";
//        $insert = $GLOBALS['db']->query($insert);
//    }
//    //print_r($_REQUEST['select2']);
//
...
function insert_role_mx($tb,$field)
{
    $array = explode(",",$field);
   ...
   // print_r($i);exit;
    ...
    $sql="insert into ".$tb."(".$f1.") values(".$f2.")";
   //print_r($sql);exit;
    $res = $GLOBALS['db']->query($sql);
...
//    }
    ...
    {
        $img_note_mx_update=" insert role_imgs(p_id,img_note_1,img_note_2,img_note_3,img_action_url,last_update) values ('".$role_sn."','".$img_note_1[$i]."','".$img_note_2[$i]."','".$img_note_3[$i]."','".$img_action_url[$i]."',);";
        
       // $img_note_mx_update=" update role_imgs set img_note_1 ='".$img_note_1[$i]."', img_note_2 ='".$img_note_2[$i]."', img_note_3 ='".$img_note_3[$i]."', img_action_url ='".$img_action_url[$i]."',last_update='".$last_update."' where img_outer_id='".$img_code[$i]."';";
...
function img_insert($arr, $obj,$tb)
{
    ...
    for ($i = 0; $i < count($obj) - 1; $i++) {
        ...
        if ($res_img[0]['sl'] == 0) {
            ...
            //$sql=" insert into role_imgs (p_id,b_img_url,s_img_url,add_time,img_sum) values ('".$arr."','".$obj[$i]."','".$s_url."','".$now."',1);";
            ...
            $outer_role_sn_2 = $res_img2[0]['img_sum'] + 1;
            //print_r($res_img2);exit;

            $sql = " insert into ".$tb." (p_id,b_img_url,s_img_url,add_time,img_sum,width,height,resize_width,resize_height) values ('" .
                $arr . "','" . $obj[$i] . "','" . $s_url[$i] . "','" . $now . "'," . $outer_role_sn_2 .
                ",'" . $obj['guige'][$i]['width'] . "','" . $obj['guige'][$i]['height'] . "','" .
                $obj['guige'][$i]['resize_width'] . "','" . $obj['guige'][$i]['resize_height'] .
                "');";
            //$sql=" insert into role_imgs (p_id,b_img_url,s_img_url,add_time,img_sum) values ('".$arr."','".$obj[$i]."','".$s_url."','".$now."',".$outer_role_sn_2.");";
            //$sql=" insert into role_images (img_role_sn,img_url,outer_role_sn) values ('".$arr."','".$obj[$i]."',".$outer_role_sn_2.");";
            //print_r($sql);exit;
            $res = $GLOBALS['db']->query($sql);
            $update = " update ".$tb." set img_outer_id=concat(p_id,'_',img_sum) where p_id='" .
                $arr . "';";
...
function get_role_imgs_list($img_tb,$img_field,$img_u_id)
{   
        function req2($obj)
    {
   ...
   // $sql = "select p_id,b_img_url,s_img_url,img_note_1,img_note_2,img_note_3,img_action_url,ss,img_outer_id,add_time,last_update  from role_imgs  where  p_id ='" . $obj . "'";
    $res = $GLOBALS['db']->getAll($sql);
    //$noNum=20;
    return array('items' => $res, 'sql' => $sql);
