...
function qudao($Nu, $tb)
{
    ...
    if (isset($_REQUEST['m_key'])) {
        $filer1 = " and ( qudao_sn like '%" . trim($_REQUEST['m_key']) .
            "%' or qudao_name like '%" . trim($_REQUEST['m_key']) . "%')";
    } else {
        $filer1 = '';
    }
    ...
    {
        $point="select * from tgpoint where p_id='".$res[$j]['qudao_sn']."' and p_type=1 order by users_sn";
...
function i_qudao($tb, $field, $time_f)
{
...
function update_qudao_mx($tb,$field,$u_id,$time_f)
{   
...
{
  $sql="select bh from api_bhwh where cj_name='qudao'";
    $res = $GLOBALS['db']->getAll($sql);
        //$noNum=20;
...
{
  $sql="update api_bhwh set bh=bh+1 where cj_name='qudao'";
  $res = $GLOBALS['db']->query($sql);
}
...
function set_point($obj,$obj2,$obj3)
{
    ...
    for($i=0;$i<count($obj);$i++)
    {
        ...
        $qrcd="select qudao_name,qrcid from qudao  where qudao_sn='".$obj3."'";
        ...
        $sql = "insert into cj_qrcode_stat(openid,issubs,cj_sn,cj_name,cj_type,qrcid,add_time,last_update_2) values('" .
                    $yonhu['openid'] . "',1,'" . $obj3 . "','" . $qrcd['qudao_name'] . "','qudao','" . $qrcd['qrcid'] .
                    "','" . $time . "','" . $last_update_2 . "')";
