...
function sales($Nu, $tb)
{
    ...
    if (isset($_REQUEST['m_key'])) {
        $filer1 = " and ( sales_sn like '%" . trim($_REQUEST['m_key']) .
            "%' or sales_name like '%" . trim($_REQUEST['m_key']) . "%')";
    } else {
        $filer1 = '';
    }
    ...
    {
        $point="select * from tgpoint where p_id='".$res[$j]['sales_sn']."' and p_type=3 order by users_sn";
...
//           // echo $filer;
//        }
  $sql="select shop_sn,shop_name,p_id from shop ";
    $res = $GLOBALS['db']->getAll($sql);
        //$noNum=20;
    return array('items'=>$res,'sql'=>$sql);
}
function get_qudao_mx($code)
{
    ...
    $sql="select qudao_sn,qudao_name,qudao_type,p_id from qudao where qudao_sn='".$code."' ";
    $res = $GLOBALS['db']->getAll($sql);
      // print_r($sql);
    return array('items'=>$res,'sql'=>$sql);
}

function get_shop_mx($code)
{
 
    $sql="select shop_sn,shop_name,shop_type,p_id from shop where shop_sn='".$code."' ";
    $res = $GLOBALS['db']->getAll($sql);
      // print_r($sql);
    return array('items'=>$res,'sql'=>$sql);
}
...
function i_sales($tb, $field, $time_f)
{
...
{   
      $sql="select id,
            ...
            last_update,
            last_update_2 ,b_id 
            from sales where sales_sn='".$obj."' ";
    $res = $GLOBALS['db']->getAll($sql);
    //$noNum=20;
    
    
    $point="select * from tgpoint where p_id='".$obj."' and p_type=3  order by users_sn";
...
function update_sales_mx($tb,$field,$u_id,$time_f)
{   
...
{
  $sql="select bh from api_bhwh where cj_name='sales'";
    $res = $GLOBALS['db']->getAll($sql);
        //$noNum=20;
    return array('items'=>$res,'sql'=>$sql);
}
...
{
  $sql="update api_bhwh set bh=bh+1 where cj_name='sales'";
  $res = $GLOBALS['db']->query($sql);
}
...
{
    $de="delete from tgpoint  where p_id='".$obj3."' and p_type=3";
    ...
    for($i=0;$i<count($obj);$i++)
    {
        ...
        $qrcd="select sales_name,qrcid from sales  where sales_sn='".$obj3."'";
        ...
        $sql = "insert into cj_qrcode_stat(openid,issubs,cj_sn,cj_name,cj_type,qrcid,add_time,last_update_2) values('" .
                    $yonhu['openid'] . "',1,'" . $obj3 . "','" . $qrcd['sales_name'] . "','sales','" . $qrcd['qrcid'] .
                    "','" . $time . "','" . $last_update_2 . "')";
                $resaa = $GLOBALS['db']->query($sql);
        
        
        $sql="insert into tgpoint(p_type,p_id,openid,users_sn,nick_name,point) select 3,'".$obj3."',openid,users_sn,nick_name,".$obj2[$i]." from users  where users_sn='".$str."'";
