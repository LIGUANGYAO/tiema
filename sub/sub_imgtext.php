...
function get_imgtext_list($Nu, $ma, $sql)
{
    ...
    if (isset($_REQUEST['m_key'])) {
        $filer1 = " and ( imgtext_sn like '%" . trim($_REQUEST['m_key']) .
            "%' or imgtext_name like '%" . trim($_REQUEST['m_key']) . "%' or imgtext_note_1 like '%" . trim($_REQUEST['m_key']) . "%' )";
    } else {
        $filer1 = '';
    }
    ...
    //$sql="select id,imgtext_sn,imgtext_name,imgtext_outer_name,imgtext_note_1,imgtext_note_2,imgtext_note_3,imgtext_note_4,imgtext_note_5,imgtext_name_eg,zz,tzsy,action_url,last_update,last_update_2,start_time,end_time,sort_no  from imgtext ";
...
function get_imgtext_mx($ma,$sql)
{   
    $ma_sn=$ma."_sn";
    ...
    //$sql = "select imgtext_sn,imgtext_name,imgtext_outer_name,imgtext_note_1,imgtext_note_2,imgtext_note_3,imgtext_note_4,imgtext_note_5,imgtext_name_eg,zz,tzsy,action_url,last_update,last_update_2,start_time,end_time,sort_no  from imgtext " 
    $sql=$sql.$where . "'";
...
function update_imgtext_mx($tb,$field,$u_id,$time_f)
{   
     //$tb,$field,$u_id表，字段，关键字
    //print_r($field);
   // $field="imgtext_name,imgtext_note_1,imgtext_note_2";
    $array = explode(",",$field);
...
//    }
    ...
    for($i=0;$i<count($img_code);$i++)
    {
        ...
        $img_note_mx_update=" update imgtext_imgs set img_note_1 ='".$img_note_1[$i]."', img_note_2 ='".$img_note_2[$i]."', img_note_3 ='".$img_note_3[$i]."', img_action_url ='".$img_action_url[$i]."',last_update='".$last_update."' where img_outer_id='".$img_code[$i]."';";
...
function insert_imgtext_mx($tb,$field,$time_f)
{
    $array = explode(",",$field);
...
//    }
    ...
    for($i=0;$i<count($img_code);$i++)
    {
        $img_note_mx_update=" insert imgtext_imgs(p_id,img_note_1,img_note_2,img_note_3,img_action_url,last_update) values ('".$imgtext_sn."','".$img_note_1[$i]."','".$img_note_2[$i]."','".$img_note_3[$i]."','".$img_action_url[$i]."',);";
...
function get_imgtext_imgs_list($img_tb,$img_field,$img_u_id)
{   
        function req2($obj)
    {
