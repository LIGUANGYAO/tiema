...
function get_custom_list($Nu, $ma, $sql)
{
    ...
    if (isset($_REQUEST['m_key'])) {
        $filer1 = "  and ( custom_sn like '%" . trim($_REQUEST['m_key']) .
            "%' or custom_name like '%" . trim($_REQUEST['m_key']) . "%' or custom_note_1 like '%" . trim($_REQUEST['m_key']) . "%' )";
    } else {
        $filer1 = '';
    }
  
     //$filer = " where  p_id is null  $filer1 ";
    ...
    //$sql="select id,custom_sn,custom_name,custom_outer_name,custom_note_1,custom_note_2,custom_note_3,custom_note_4,custom_note_5,custom_name_eg,zz,tzsy,action_url,last_update,last_update_2,start_time,end_time,sort_no  from custom ";
    $sql = $sql . $filer . " order by p_id,-sort_no,id desc  " . $obj['limit_obj'] . ";";
...
{   
    ...
    //$sql = "select custom_sn,custom_name,custom_outer_name,custom_note_1,custom_note_2,custom_note_3,custom_note_4,custom_note_5,custom_name_eg,zz,tzsy,action_url,last_update,last_update_2,start_time,end_time,sort_no  from custom " 
    $sql=$sql.$where . "'";
...
function update_custom_mx($tb,$field,$u_id,$time_f)
{   
     //$tb,$field,$u_id表，字段，关键字
    //print_r($field);
   // $field="custom_name,custom_note_1,custom_note_2";
    $array = explode(",",$field);
...
//    {
//        
//        $img_note_mx_update=" update custom_imgs set img_note_1 ='".$img_note_1[$i]."', img_note_2 ='".$img_note_2[$i]."', img_note_3 ='".$img_note_3[$i]."', img_action_url ='".$img_action_url[$i]."',last_update='".$last_update."' where img_outer_id='".$img_code[$i]."';";
//        
...
function insert_custom_mx($tb,$field,$time_f)
{
    $array = explode(",",$field);
...
//    }
    ...
    for($i=0;$i<count($img_code);$i++)
    {
        $img_note_mx_update=" insert custom_imgs(p_id,img_note_1,img_note_2,img_note_3,img_action_url,last_update) values ('".$custom_sn."','".$img_note_1[$i]."','".$img_note_2[$i]."','".$img_note_3[$i]."','".$img_action_url[$i]."',);";
...
function img_insert($arr, $obj,$tb)
{
    ...
    for ($i = 0; $i < count($obj) - 1; $i++) {
        ...
        if ($res_img[0]['sl'] == 0) {
            ...
            //$sql=" insert into custom_imgs (p_id,b_img_url,s_img_url,add_time,img_sum) values ('".$arr."','".$obj[$i]."','".$s_url."','".$now."',1);";
            ...
            $outer_custom_sn_2 = $res_img2[0]['img_sum'] + 1;
            //print_r($res_img2);exit;

            $sql = " insert into ".$tb." (p_id,b_img_url,s_img_url,add_time,img_sum,width,height,resize_width,resize_height) values ('" .
                $arr . "','" . $obj[$i] . "','" . $s_url[$i] . "','" . $now . "'," . $outer_custom_sn_2 .
                ",'" . $obj['guige'][$i]['width'] . "','" . $obj['guige'][$i]['height'] . "','" .
                $obj['guige'][$i]['resize_width'] . "','" . $obj['guige'][$i]['resize_height'] .
                "');";
            //$sql=" insert into custom_imgs (p_id,b_img_url,s_img_url,add_time,img_sum) values ('".$arr."','".$obj[$i]."','".$s_url."','".$now."',".$outer_custom_sn_2.");";
            //$sql=" insert into custom_images (img_custom_sn,img_url,outer_custom_sn) values ('".$arr."','".$obj[$i]."',".$outer_custom_sn_2.");";
            //print_r($sql);exit;
            $res = $GLOBALS['db']->query($sql);
            $update = " update ".$tb." set img_outer_id=concat(p_id,'_',img_sum) where p_id='" .
                $arr . "';";
...
function get_custom_imgs_list($img_tb,$img_field,$img_u_id)
{   
        function req2($obj)
    {
   ...
   // $sql = "select p_id,b_img_url,s_img_url,img_note_1,img_note_2,img_note_3,img_action_url,ss,img_outer_id,add_time,last_update  from custom_imgs  where  p_id ='" . $obj . "'";
    $res = $GLOBALS['db']->getAll($sql);
    //$noNum=20;
    return array('items' => $res, 'sql' => $sql);
