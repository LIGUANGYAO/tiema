...
function get_fcsend_list($Nu, $ma, $sql)
{
    ...
    $sql_c = "select a.*,from_unixtime(a.add_time) as time,b.openid,b.nick_name,b.sex,c.openid as p_openid2,c.nick_name as p_nick_name,c.sex as p_sex from  wxpay_fclog a inner join users b on a.openid=b.openid inner join users c on a.p_openid =c.openid ".$filer." order by a.add_time  desc ";
    //echo $sql_c;
...
function get_fcsend_mx($ma, $sql)
{
    ...
    //$sql = "select fcsend_sn,fcsend_name,fcsend_outer_name,fcsend_note_1,fcsend_note_2,fcsend_note_3,fcsend_note_4,fcsend_note_5,fcsend_name_eg,zz,tzsy,action_url,last_update,last_update_2,start_time,end_time,sort_no  from fcsend "
    $sql = $sql . $where . "'";
...
function update_fcsend_mx($tb, $field, $u_id, $time_f)
{
    //$tb,$field,$u_id表，字段，关键字
    //print_r($field);
    // $field="fcsend_name,fcsend_note_1,fcsend_note_2";
    $array = explode(",", $field);
    ...
    for ($i = 0; $i < count($img_code); $i++) {
        ...
        $img_note_mx_update = " update fcsend_imgs set img_note_1 ='" . $img_note_1[$i] .
...
function insert_fcsend_mx($tb, $field, $time_f)
{
    $array = explode(",", $field);
    ...
    for ($i = 0; $i < count($img_code); $i++) {
        $img_note_mx_update =
            " insert fcsend_imgs(p_id,img_note_1,img_note_2,img_note_3,img_action_url,last_update) values ('" .
            $fcsend_sn . "','" . $img_note_1[$i] . "','" . $img_note_2[$i] . "','" . $img_note_3[$i] .
            "','" . $img_action_url[$i] . "',);";

        // $img_note_mx_update=" update fcsend_imgs set img_note_1 ='".$img_note_1[$i]."', img_note_2 ='".$img_note_2[$i]."', img_note_3 ='".$img_note_3[$i]."', img_action_url ='".$img_action_url[$i]."',last_update='".$last_update."' where img_outer_id='".$img_code[$i]."';";
...
function img_insert($arr, $obj, $tb)
{
    ...
    for ($i = 0; $i < count($obj) - 1; $i++) {
        ...
        if ($res_img[0]['sl'] == 0) {
            ...
            //$sql=" insert into fcsend_imgs (p_id,b_img_url,s_img_url,add_time,img_sum) values ('".$arr."','".$obj[$i]."','".$s_url."','".$now."',1);";
            $sql = " insert into " . $tb .
            ...
            $outer_fcsend_sn_2 = $res_img2[0]['img_sum'] + 1;
            //print_r($res_img2);exit;

            $sql = " insert into " . $tb .
                " (p_id,b_img_url,s_img_url,add_time,img_sum,width,height,resize_width,resize_height) values ('" .
                $arr . "','" . $obj[$i] . "','" . $s_url[$i] . "','" . $now . "'," . $outer_fcsend_sn_2 .
                ",'" . $obj['guige'][$i]['width'] . "','" . $obj['guige'][$i]['height'] . "','" .
                $obj['guige'][$i]['resize_width'] . "','" . $obj['guige'][$i]['resize_height'] .
                "');";
            //$sql=" insert into fcsend_imgs (p_id,b_img_url,s_img_url,add_time,img_sum) values ('".$arr."','".$obj[$i]."','".$s_url."','".$now."',".$outer_fcsend_sn_2.");";
            //$sql=" insert into fcsend_images (img_fcsend_sn,img_url,outer_fcsend_sn) values ('".$arr."','".$obj[$i]."',".$outer_fcsend_sn_2.");";
            //print_r($sql);exit;
            $res = $GLOBALS['db']->query($sql);
            $update = " update " . $tb .
                " set img_outer_id=concat(p_id,'_',img_sum) where p_id='" . $arr . "';";
...
function get_fcsend_imgs_list($img_tb, $img_field, $img_u_id)
{
    function req2($obj)
    {
        if (isset($_REQUEST[$obj])) {
    ...
    $sql = "select " . $img_field . " from " . $img_tb . " where p_id='" . $img_u_id .
        "'";
    // $sql = "select p_id,b_img_url,s_img_url,img_note_1,img_note_2,img_note_3,img_action_url,ss,img_outer_id,add_time,last_update  from fcsend_imgs  where  p_id ='" . $obj . "'";
    $res = $GLOBALS['db']->getAll($sql);
    //$noNum=20;
    return array('items' => $res, 'sql' => $sql);
