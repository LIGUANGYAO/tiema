...
require (dirname(__file__) . '/sub/sub_wx_lv_msg.php');
...
if ($_REQUEST['act'] == 'default') {
    ...
    //$sql = "SELECT a.*,b.nick_name FROM wx_lv_msg  a inner join users  b on a.openid=b.openid  ";
    ...
    $sql="SELECT a.*,b.nick_name,c.cj_sn,c.cj_name,c.cj_type,case when c.cj_type ='qudao' then '渠道' when c.cj_type ='shop' then '商店' when c.cj_type ='sales' then '导购' else '微信公众号' end as type,a.tzsy FROM wx_lv_msg  a inner join users  b on a.openid=b.openid left join cj_qrcode_stat c on a.openid=c.openid";
    $wx_lv_msg_list = get_wx_lv_msg_list($Num, "weixiaodian", $sql);
    //print_r($wx_lv_msg_list);

    

    $smarty->assign('wx_lv_msg_list', $wx_lv_msg_list['items']);
    $smarty->assign('fall', 1);
    $smarty->assign('title', $aaa);
    $smarty->assign('p_Array', $wx_lv_msg_list['page']);
    //添加时间间隔
    ...
    $smarty->display('wx_lv_msg.tpl');
}

if ($_REQUEST['act'] == 'add_wx_lv_msg_list') {

    //$sql="select xmlx_sn ,xmlx_name from xmlx";
    //   $res_xmlx = $GLOBALS['db']->getAll($sql);
    //    $smarty->assign('res_xmlx', $res_xmlx);
    $smarty->assign('fall', 'add_wx_lv_msg_list');
    $smarty->display('wx_lv_msg_mx.tpl');
}
...
if ($_REQUEST['act'] == 'edit') {
    ...
    ///修改2,查询语句
    $sql = "select id,wx_lv_msg_sn,wx_lv_msg_name,sort_no,tzsy,last_update,url,type
  from wx_lv_msg ";
    $wx_lv_msg_mx = get_wx_lv_msg_mx("wx_lv_msg", $sql);
    // print_r($wx_lv_msg_mx);exit;
    $img_cod = $_REQUEST['wx_lv_msg_sn']; //    //图片部分。没图片则删除
    //    $wx_lv_msg_imgs2 = get_wx_lv_msg_imgs_list("wx_lv_msg_imgs"," p_id,b_img_url,s_img_url,img_note_1,img_note_2,img_note_3,img_action_url,ss,img_outer_id,add_time,last_update",$img_cod);
    ////print_r($wx_lv_msg_imgs2);
    //    $wx_lv_msg_imgs = arr_push($wx_lv_msg_imgs2['items']);
    //    $smarty->assign('wx_lv_msg_imgs', $wx_lv_msg_imgs);


    $smarty->assign('wx_lv_msg_mx', $wx_lv_msg_mx['items'][0]);
    $smarty->assign('res_xmlx', $wx_lv_msg_mx['res_xmlx']);
...
if ($_REQUEST['act'] == 'delete') {
    ...
    if (isset($_REQUEST['img_code'])) {
        ...
        $sql = "delete from wx_lv_msg_imgs where  img_outer_id= '" . $img_code . "'";
...
if ($_REQUEST['act'] == 'delete_wx_lv_msg') {

    //echo 1;
    if (isset($_REQUEST['wx_lv_msg_sn'])) {
        $wx_lv_msg_sn = trim($_REQUEST['wx_lv_msg_sn']);
        $sql = "delete from wx_lv_msg where  wx_lv_msg_sn= '" . $wx_lv_msg_sn . "'";
        $res = $GLOBALS['db']->query($sql);
        echo "删除成功";
    } else {
        echo "失败";
...
if ($_REQUEST['act'] == 'img_xs') {
    ...
    if (isset($_REQUEST['img_code']) && isset($_REQUEST['alt'])) {
        ...
        $sql = "update  wx_lv_msg_imgs set ss=" . $alt . "  where  img_outer_id= '" . $img_code .
            "'";
        $res = $GLOBALS['db']->query($sql);
        echo "修改成功";
    } else {
...
if ($_REQUEST['act'] == 'wx_lv_msg_xs') {

    //echo 1;

    if (isset($_REQUEST['wx_lv_msg_code']) && isset($_REQUEST['alt'])) {
        $wx_lv_msg_code = urldecode(trim($_REQUEST['wx_lv_msg_code']));
        $alt = urldecode(trim($_REQUEST['alt']));
        $sql = "update  wx_lv_msg set tzsy=" . $alt . "  where  openid= '" . $wx_lv_msg_code .
            "' and tzsy=0";
        $res = $GLOBALS['db']->query($sql);
        echo $sql;
        echo "修改成功";
...
if ($_REQUEST['act'] == 'post') {
    ...
    //$p = new upload;
    //    $path=$p->upload_path='upload/wx_lv_msg';
    //    $p->uplood_img();
    ...
    //    }
    //    $aaa = $_POST['wx_lv_msg_sn'];
    //    // print_r($aaa);exit;
    //    //图片部分。没图片则删除
    //    img_insert($aaa, $img_array,"wx_lv_msg_imgs");
    //修改3，更新语句
    ...
    update_wx_lv_msg_mx("wx_lv_msg", "wx_lv_msg_name,url,type,sort_no", "wx_lv_msg_sn", $time_field);
...
if ($_REQUEST['act'] == 'post_add') {
    ...
    $get_one = " select wx_lv_msg_sn from wx_lv_msg where wx_lv_msg_sn ='" . $wx_lv_msg_sn .
        "'";
    ...
    if (empty($res)) {
        ...
        //保存修改后商品明细
        insert_wx_lv_msg_mx("wx_lv_msg", "wx_lv_msg_sn,wx_lv_msg_name,url,type,sort_no", $time_field);
        $smarty->assign('fall', 'post');
        $smarty->display('wx_lv_msg_mx.tpl');
    } else {
...
if ($_REQUEST['act'] == 'down_dj') {
    ...
    $smarty->assign('th_time', $th_time);
    $smarty->assign('now_time', $time);
    $smarty->display('wx_lv_msg_xz.html');
}
...
if ($_REQUEST['act'] == 'down_order') {
    ...
    function wx_lv_msg_order($obj)
    {
        ...
        for ($i = 0; $i < count($obj2); $i++) {
            ...
            $sel = "select order_sn from wx_lv_msg where order_sn='" . $obj2[$i]['order_id'] .
                "'"; // print_r($sel);
            ...
            if (empty($sel)) {
                ...
                $up = "update wx_lv_msg set order_status='" . $obj2[$i]['order_status'] .
    ...
    for ($i = 0; $i < count($status); $i++) {
        ...
        if (!empty($order_list['order_list'])) {
            ...
            $msg = wx_lv_msg_order($order_list);
            $xx .= $msg;
        }
