...
if ($_REQUEST['act'] == 'default') {
    ...
    $activity = "select a.activity_sn,a.activity_name,a.tzsy,a.add_time,a.limit_time,a.hd_number,a.start_time,a.end_time,b.* from activity a , activity_mx b  where a.ac_lx=3 and a.tzsy=0 and (a.start_time<='" .
        $whell . "' and a.end_time>'" . $whell .
        "') and a.activity_sn=b.p_id order by a.sort_no desc";
    ...
    if (empty($activity)) {
        ...
        $data1 = array("error" => "activend", "msg" => "该时段无刮刮卡活动,活动时间请留意微信公众号");
    ...
    $tot = ($activity[0]['prize1_sl'] - $activity[0]['prize1_sl2']) + ($activity[0]['prize2_sl'] -
        $activity[0]['prize2_sl2']) + ($activity[0]['prize3_sl'] - $activity[0]['prize3_sl2']);
    ...
    //print_r(getReward(1000,60,10,10)) ;exit;
    ...
    if (isset($_REQUEST['openid'])) {
        ...
        $limittime = unittodate($time3, $limit_time); //到期时间
        ...
        if ($today_sum['today_sum'] < $today_sum['hd_lo_sum']) {
            ...
            if ($data == 1) {
                    ...
                    if (!empty($sel['wx_tel'])) {
                        ...
                        for ($i = 0; $i < count($sncode_list); $i++) {
                            ...
                            $os_sql = "insert into shop_vouchers (pr_id,card_sn,pwd,start_time,end_time,provide_time,user_id,cv_amount,lowest_amount,is_active,sendTo) values ('10','" .
                                ...
                                "','" . $sncode_list[$i]['limit_time'] . "','" . $sncode_list[$i]['add_time'] .
                ...
                if (CUSTOMER == 'CASLON' && U_DB2 == 1) {
                    ...
                    $data1 = array(
                        ...
                        "c_sum" => $today_sum['hd_lo_sum']);
                    $sql3 = "insert into wx_sncode(openid,sncode,prizetype,add_time,limit_time,hd_sn,lo_date) values ('" .
                        $openid . "','" . $lo_sn . "','2','" . $time2 . "','" . $limittime . "','" . $ac_sn .
                        "','" . $time . "')";
                    $sql3 = $GLOBALS['db']->query($sql3);
                ...
                if (CUSTOMER == 'CASLON' && U_DB2 == 1) {
                    ...
                    $sql3 = "insert into wx_sncode(openid,sncode,prizetype,add_time,limit_time,hd_sn,lo_date) values ('" .
                        $openid . "','" . $lo_sn . "','3','" . $time2 . "','" . $limittime . "','" . $ac_sn .
                        "','" . $time . "')";
                    $sql3 = $GLOBALS['db']->query($sql3);
            ...
            //echo "超过";
            $data1 = array("error" => "invalid", "msg" => "您今天已经抽了 " . $hd_number .
                    " 次奖,不能再抽了,明天再来吧!");
            $aaa = new arraytojson();
            $json = $aaa->JSON($data1);
            print_r($json);
...
if ($_REQUEST['act'] == 'setTel') {
    ...
    if (isset($_REQUEST['tel'])) {
        ...
        if (isset($code)) {
            ...
            $filename = $PNG_TEMP_DIR . 'test' . md5($code . '|' . $errorCorrectionLevel .
                '|' . $matrixPointSize) . '.png';
        ...
        $temp_qrcode = 'test' . md5($code . '|' . $errorCorrectionLevel . '|' . $matrixPointSize) .
            '.png';
        //display generated file
        //echo '<img src="'.$PNG_WEB_DIR.basename($filename).'" /><hr/>';
        $up_lo = "update wx_sncode set tel='" . $tel . "',temp_qrcode='" . $temp_qrcode .
            "' where   openid='" . $openid . "' and lo_date='" . $time . "' and sncode='" .
            $code . "'";
        $up_lo = $GLOBALS['db']->query($up_lo);
        $data1 = array(
            "success" => true,
