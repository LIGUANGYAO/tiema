...
{if $fall==1}
 ...
 <form>
 <tr>
 <th style="text-align: left;border-right-style:none"><input id="a_text" value="添加分类" type="button" onclick="location='wxfloor2.php?act=add_wxfloor2_list'" />&nbsp;<input id="down_hy" value="头像下载" type="hidden" /></th>
...
<tbody  id="table_t">
   {foreach from=$wxfloor2_list item=wxfloor2_list}
  <tr>
  <td style="width:70px">{$wxfloor2_list.wxfloor2_sn}</td>
  <td style="width:140px">{$wxfloor2_list.wxfloor2_name}</td>
  <td  title="{$wxfloor2_list.wxfloor2_note_1}">{$wxfloor2_list.wxfloor2_note_1}</td>
   <td style="width:70px">{if $wxfloor2_list.type==1}信用卡使用及提额宝典{/if}</td>
  
   <td style="width:140px">{$wxfloor2_list.wxfloor_sn}_{$wxfloor2_list.wxfloor_name}</td>
  <td style="width:150px">{$wxfloor2_list.sort_no}</td>
 <td style="width:150px">{$wxfloor2_list.last_update}</td>

  
  <td style="width:90px"> 
  {if $wxfloor2_list.tzsy==1}
   <img  title="启用" name="{$wxfloor2_list.wxfloor2_sn}" id="tzsy_qy"  src="images/no.gif" />&nbsp;
  <img  title="修改" name="{$wxfloor2_list.wxfloor2_sn}" id="edit"  src="images/icon_edit.gif" />&nbsp;
  <img id="delete_wxfloor2" name="{$wxfloor2_list.wxfloor2_sn}" title="删除" src="images/icon_drop.gif"/>
  {else if }
  <!--<img  title="发送预览" wxfloor2_sn="{$wxfloor2_list.wxfloor2_sn}" wxfloor2_name="{$wxfloor2_list.wxfloor2_name}"  class="send"  src="images/send.gif" />&nbsp;-->
  <img  title="禁用" name="{$wxfloor2_list.wxfloor2_sn}" id="tzsy_jy"  src="images/yes.gif" />&nbsp;
  <img  title="修改" name="{$wxfloor2_list.wxfloor2_sn}" id="edit"  src="images/icon_edit.gif" />&nbsp;
  <img id="delete_wxfloor2" name="{$wxfloor2_list.wxfloor2_sn}" title="删除" src="images/icon_drop.gif"/>
  {/if}
  </td>
     ...
     $("#text_sn").focus();
       $("#queding").click(function(){
        ...
        $("img[id='edit']").each(function (){
           ...
           window.location='wxfloor2.php?act=edit&wxfloor2_sn='+encodeURI(encodeURI($(this).attr("name")));
         ...
         $("img[id='delete_wxfloor2']").each(function (){
            
            $(this).click(function (){
        
            
              if(confirm("删除"+$(this).attr("name")+"")){
               htmlobj=$.ajax({url:"wxfloor2.php?act=delete_wxfloor2&wxfloor2_sn="+encodeURI(encodeURI($(this).attr("name"))),async:false});
        ...
        $("img[id='tzsy_jy']").each(function (){
           ...
           if(confirm("禁用"+$(this).attr("name")+"")){
              ...
              htmlobj=$.ajax({url:"wxfloor2.php?act=wxfloor2_xs&wxfloor2_code="+encodeURI(encodeURI($(this).attr("name")))+"&alt=1",async:false});
              //alert(htmlobj.responseText);
             window.location.reload();
            }else return false;
        ...
        $("img[id='tzsy_qy']").each(function (){
           ...
           if(confirm("启用"+$(this).attr("name")+"")){
              ...
              htmlobj=$.ajax({url:"wxfloor2.php?act=wxfloor2_xs&wxfloor2_code="+encodeURI(encodeURI($(this).attr("name")))+"&alt=0",async:false});
              //alert(htmlobj.responseText);
             window.location.reload();
            }else return false;
         ...
         $(".send").each(function(){
             ...
             $(this).click(function(){
                ...
                $("#send_sn").text($(this).attr("wxfloor2_sn"));
                
                $("#send_name").text($(this).attr("wxfloor2_name"));
