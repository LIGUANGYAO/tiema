    ...
    if (isset($_REQUEST['openid'])) {
        ...
        for ($i = 0; $i < count($get_point); $i++) {
            if ($get_point[$i]['p_type'] == "1") {
                ...
                $get_point[$i]['shop_sales_fenc'] = shop_to_sales($res['shop_sn']);
                //--end结束下属商店结算
                // print_r($get_point);
     ...
     function fenc_3($p_id, $tp, $fenc)
    {
        ...
        $res = $GLOBALS['db']->getAll($sql_g);
                ...
                for ($j = 0; $j < count($res); $j++) {
                   ...
                   $res[$j]['fc'] =round( $fenc/100 /round($res[$j]['product_price']/$res[$j]['order_total_price'],2) ,2)  * $res[$j]['product_price'];
                   
                   $res[$j]['djfc'] =round($fenc/100 /round($res[$j]['product_price']/$res[$j]['order_total_price'],2) ,2)  * $res[$j]['product_price'];
...
if ($_REQUEST['act'] == 'order_search') {
    ...
    if (CUS == 'YATE' && U_DB2 == 11) {
        ...
        {
            $sql = "select *,from_unixtime(order_create_time) as add_t from weixiaodian where buyer_openid='".$obj."' order by from_unixtime(order_create_time) desc ";
            $res = $GLOBALS['db']->getAll($sql);   
            return $res;
        }
