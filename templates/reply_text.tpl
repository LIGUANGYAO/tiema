...
新建<img  id="add_images" src="images/icon_add.gif" onclick="location='reply_text.php?action=addnews'"/>
</form>
	</div></td></tr>
	</table>
...
<tbody id="tbody_1">
  {foreach from=$news item=news}
			...
			<td >{$news.id}</td>
            <td ><a href="html/{$news.htmlurl}">{$news.title}</a></td>
            <td >{$news.bid}</td>
            
            <td>{$news.last_update}1</td>
            <td>{$news.add_time}</td>
            <td> <a href="reply_text.php?action=update&id={$news.id}">修改</a>  <a href="reply_text.php?action=delete&id={$news.id}">删除</a></td>
