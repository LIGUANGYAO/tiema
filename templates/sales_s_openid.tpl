...
$("#ysss").click(function ()
		...
		var s="";
		 ...
		 if( window.opener.document.all.p_id.value!='')
		  ...
		  htmlobj=$.ajax({url:"sales.php?act=ysss&type={$type}&s="+s+"&keyword="+encodeURI(encodeURI($("#b_ysss").val())),async:false,dataType: 'json'});
		  }
		  else
		  {
		  htmlobj=$.ajax({url:"sales.php?act=ysss&type={$type}&keyword="+encodeURI(encodeURI($("#b_ysss").val())),async:false,dataType: 'json'});
		  }
