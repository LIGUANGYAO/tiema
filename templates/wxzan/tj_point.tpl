...
<tbody  id="table_t">
  ...
  <tr>
  <td >{if $tj_point_list.cj_type=='qudao'}渠道{elseif $tj_point_list.cj_type=='shop'}商店{elseif $tj_point_list.cj_type=='sales'}店员{else}{/if}</td>
  <td >{$tj_point_list.qudao_sn}_{$tj_point_list.qudao_name}</td>
  
  <td >{foreach from=$tj_point_list.tg item=tg}<p>{$tg.users_sn}({$tg.nick_name})</p> {/foreach}</td>
  <td >
  {foreach from=$tj_point_list.tg item=tg}<p>{$tg.point}%</p> {/foreach}
  </td>

    <td >
  {foreach from=$tj_point_list.tg item=tg}<p>￥{$tg.fc}</p> {/foreach}
  </td>
  <td >{$tj_point_list.zbl}%/￥{$tj_point_list.zfc}</td>
  
   <td >{if $tj_point_list.p_tp==1}{$tj_point_list.shop_fenc.zongfc}{elseif $tj_point_list.p_tp==2}无{else}无{/if}</td>
   <td >
   {if $tj_point_list.p_tp==1}{$tj_point_list.sales_fenc.zongfc}{elseif $tj_point_list.p_tp==2}{$tj_point_list.shop_sales_fenc.zongfc}{else}无{/if}
   </td>
   
  <td >{$tj_point_list.order_sn}</td>
  <td >{$tj_point_list.order_total_price}</td>
  <td >{$tj_point_list.product_price}</td>
  <td >{$tj_point_list.order_express_price}</td>
  <td >{$tj_point_list.buyer_nick}</td>
  <!--<td >{$tj_point_list.buyer_openid}</td>-->
   <td >{$tj_point_list.order_create_time}</td>
    <td >{if $tj_point_list.order_status==2}待发货{elseif $tj_point_list.order_status==3}已发货{elseif $tj_point_list.order_status==5}已完成{elseif $tj_point_list.order_status==8}<span style="color:red">维权</span>{else}{/if} </td >
  </tr>
  {foreachelse}
  <tr>
...
{       
	...
	order_status = $("#order_status").val();
	 ...
	 window.open("excel.php?m=tj_point&t1="+time+"&t2="+time2+"&order_status="+order_status); 
        })
})
-->
</script>
