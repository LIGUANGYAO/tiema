...
<tbody  id="table_t">
   {foreach from=$schedule_list item=schedule_list}
  <tr>
  
  
   <td ><p>{$schedule_list.type}</p><p>{$schedule_list.cj_sn}/{$schedule_list.cj_name}</p></td>

  <td  style="width:100px">{$schedule_list.nick_name}<!--<p>{$schedule_list.openid}</p>--></td>
  <td ><p>{$schedule_list.name}</p><p>{$schedule_list.tel}</p></td>
 
  <td >{$schedule_list.email}</td>
  <td >{$schedule_list.address}</td>
  <td >{$schedule_list.note}</td>
  <td >{$schedule_list.add_time}</td>
    <td style="width:50px"> 
  {if $schedule_list.tzsy==0}
  <img  title="审核" name="{$schedule_list.nick_name}" op="{$schedule_list.openid}" openid="{$schedule_list.openid}" nick_name="{$schedule_list.nick_name}" id="tzsy_jy"  src="images/yanshou.png" />&nbsp;
  <img src="images/icon_title.gif" class="lmsg"  openid="{$schedule_list.openid}" nick_name="{$schedule_list.nick_name}" title="留言" />
  {else if }
    <img src="images/icon_title.gif" class="lmsg"  openid="{$schedule_list.openid}" nick_name="{$schedule_list.nick_name}" title="留言" />
  {/if}
  </td>
...
{       
     $("#text_sn").focus();
       $("#queding").click(function(){
        ...
        $("img[id='edit']").each(function (){
           ...
           window.location='schedule.php?act=edit&schedule_sn='+encodeURI(encodeURI($(this).attr("name")));
         ...
         $("img[id='delete_schedule']").each(function (){
            
            $(this).click(function (){
        
            
              if(confirm("删除"+$(this).attr("name")+"")){
               htmlobj=$.ajax({url:"schedule.php?act=delete_schedule&schedule_sn="+encodeURI(encodeURI($(this).attr("name"))),async:false});
        ...
        $("img[id='tzsy_jy']").each(function (){
           ...
           if(confirm("审核 "+$(this).attr("name")+" 的留言")){
                ...
                htmlobj=$.ajax({url:"schedule.php?act=schedule_xs&schedule_code="+encodeURI(encodeURI($(this).attr("op")))+"&alt=1",async:false});
              //alert(htmlobj.responseText);
              //window.location.reload();
                var h1= 80;
        ...
        $("img[id='tzsy_qy']").each(function (){
           ...
           if(confirm("启用"+$(this).attr("name")+"")){
              ...
              htmlobj=$.ajax({url:"schedule.php?act=schedule_xs&schedule_code="+encodeURI(encodeURI($(this).attr("name")))+"&alt=0",async:false});
              //alert(htmlobj.responseText);
             window.location.reload();
            }else return false;
	...
	time2 = $(".t2").val();
	 ...
	 window.open("excel.php?m=schedule&t1="+time+"&t2="+time2); 
        })
