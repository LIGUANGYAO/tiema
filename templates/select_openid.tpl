...
$("#ysss").click(function ()
		...
		var s="";
		 ...
		 if( window.opener.document.all.qudao.value!='')
		  ...
		  htmlobj=$.ajax({url:"qudao.php?act=ysss&type={$type}&s="+s+"&keyword="+encodeURI(encodeURI($("#b_ysss").val())),async:false,dataType: 'json'});
		  }
		  else
		  {
		  htmlobj=$.ajax({url:"qudao.php?act=ysss&type={$type}&keyword="+encodeURI(encodeURI($("#b_ysss").val())),async:false,dataType: 'json'});
		  }
