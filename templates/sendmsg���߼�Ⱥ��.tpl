...
<tbody  id="table_t">
  ...
  <tr>
 <!-- <td style="width:100px">{if $sendmsg_list.qf_type=="1"}<a style="color:green">普通</a>{elseif $sendmsg_list.qf_type=="2"}<a style="color:red">高级(每月4次)</a>{else}{/if}</td>-->
  <td style="width:70px">{$sendmsg_list.sendmsg_sn}</td>
  <td style="width:140px">{$sendmsg_list.sendmsg_name}</td>
  <td style="width:100px">{if $sendmsg_list.re_type=="text"}文本__{$sendmsg_list.re_code}{elseif $sendmsg_list.re_type=="imgtext"}图文__{$sendmsg_list.re_code}{else}{/if}</td>
  <td  title="">
