...
drawLinks();});};
/*getSortData*/
(function(){$.getJSONP("http://www.360buy.com/lishi.aspx?callback=getBrandService&id=a,915,925^b,916,926^c,917,927^d,918,928^e,919,929^f,920,930^g,921,931^h,922,932^i,923,933^j,924,934",getBrandService);})();function getBrandService(json){var data=json.data;$.each(data,function(i){var html="<dl class=\"brands\"><dt>推荐品牌</dt><dd>";var d1=data[i]["b"],d2=data[i]["p"];if (d1.length>0){$.each(d1,function(j){html+="<em><a href=\""+ d1[j]["u"] +"\">"+ d1[j]["n"] +"</a></em>";});}else{html+="<div class='norecode'>暂无推荐品牌！</div>";}html+="</dd></dl><dl class=\"promotion\"><dt>促销用户</dt><dd><ul>";if (d2.length>0){$.each(d2,function(k){html+="<li>·<a href=\""+ d2[k]["u"] +"\">"+ d2[k]["n"] +"</a></li>";});}else{html+="<div class='norecode'>暂无促销用户！</div>";}html+="</ul></dd></dl>";$("#JD_sort_"+data[i]["id"]).html(html);});}
/*jdThickbox*/
