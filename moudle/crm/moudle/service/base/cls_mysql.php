 ...
 *   @package     eBusiness
 *   @version     $Id: cls_mysql.php,v 1.0 2010/05/24 07:35:58 modified $
 *   @author      malongfei <mlf@baisonmail.com>
 */
...
//    var $max_cache_time = 300; // 最大的缓存时间，以秒为单位
    ...
    function cls_mysql($dbhost, $dbuser, $dbpw, $dbname = '', $charset = 'gbk', $pconnect = 0, $quiet = 0)
    {
        ...
        if (defined('SOURCE_PATH') && !$this->root_path)
        {
            $this->root_path = SOURCE_PATH;
        }
    ...
    {
				//error_log("\n====\n".var_export($sql,true)."\n----\n",3,ROOT_PATH.'/data/error_mysql.log');
    ...
    {
    	return $this->autoExecute($table, $field_values, 'UPDATE',$where);
    }
	function insert($table, $field_values)
    {
    	return $this->autoExecute($table, $field_values);
    }
    ...
    function autoExecute($table, $field_values, $mode = 'INSERT', $where = '', $querymode = '')
    {
        ...
        if ($mode == 'INSERT')
        {
            ...
            foreach ($field_names AS $value)
            {
                ...
                {
                    $fields[] = $value;
                    $values[] = "'" . trim(addslashes($field_values[$value])) . "'";
                }
            }
        ...
        else
        {
            ...
            foreach ($field_names AS $value)
            {
                if (array_key_exists($value, $field_values) == true)
                {
                    $sets[] = $value . " = '" . trim(addslashes($field_values[$value])) . "'";
                }
            }
