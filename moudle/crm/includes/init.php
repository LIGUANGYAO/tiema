...
function log_out()
{
    ...
    setcookie("Login_wx_name", $session_un, time() - 999);
     setcookie("Login_app", $session_un, time() - 999);
     setcookie("Login_qd", $session_un, time() - 999);
}
...
function getlogin_name()
{
    ...
    $tmpUnSerialize=base64_decode($tmpUnSerialize);
    return $tmpUnSerialize;
}
function getlogin_app()
{
    $cookie = stripslashes($_COOKIE['Login_app']);

    $tmpUnSerialize = unserialize($cookie);
    $tmpUnSerialize=base64_decode($tmpUnSerialize);
    return $tmpUnSerialize;
}
function getlogin_qd()
{
    $cookie = stripslashes($_COOKIE['Login_qd']);
...
//echo getlogin_name();
function action_log($action_sn, $action_name,$name,$ip, $bz = '', $djbh = '')
{
    $t = date('Y-m-d H:i:s', time());
    $sql = "insert into action_log(action_sn,action_name,rq,user_sn,bz,djbh,ip) values ('" .
        $action_sn . "','" . $action_name . "','" . $t . "','" . $name . "','" . $bz .
        "','" . $djbh . "','" . $ip . "')";
    $res = $GLOBALS['db']->query($sql);
}


function dj_log($action_sn, $action_name, $bz = '', $djbh = '')
{
    $t = date('Y-m-d H:i:s', time());
    $sql = "insert into dj_log(action_sn,action_name,rq,user_sn,bz,djbh,ip) values ('" .
        $action_sn . "','" . $action_name . "','" . $t . "','" . getlogin_name() . "','" . $bz .
        "','" . $djbh . "','" . get_real_ip() . "')";
    $res = $GLOBALS['db']->query($sql);
}
