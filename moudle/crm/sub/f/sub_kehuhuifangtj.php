...
function get_kehuhuifangtj_list($Nu, $ma, $sql)
{
    ...
    $getcount="select count(*) as sl from kehuhuifang a inner join kehu b  on a.kehu_id=b.id inner join admin_user c on a.user_name=c.user_name ".$filer;
    $getcount = $GLOBALS['db']->getRow($getcount);
    $res2=$getcount['sl'];
    ...
    $sql = $sql . $filer . " order by a.id desc " . $obj['limit_obj'] . ";";
...
function get_kehuhuifangtj_mx($ma,$sql)
{   
...
function update_kehuhuifangtj_mx($tb,$field,$u_id,$time_f)
{   
...
function insert_kehuhuifangtj_mx($tb,$field,$time_f)
{
    $array = explode(",",$field);
