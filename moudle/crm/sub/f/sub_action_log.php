...
function get_action_log_list($Nu, $ma, $sql)
{
    ...
    if (isset($_REQUEST['m_key'])) {
        ...
        $f2="";
        $f2.="action_sn like '%" . trim($_REQUEST['m_key']) . "%' or ";
   
        $f2.="action_name like '%" . trim($_REQUEST['m_key']) . "%' or ";
   
        $f2.="user_sn like '%" . trim($_REQUEST['m_key']) . "%' or ";
   
        $f2.="ip like '%" . trim($_REQUEST['m_key']) . "%' ";
    ...
    if(isset($_REQUEST['rq_1']) and isset($_REQUEST['rq_2']))
    {
        if(trim($_REQUEST['rq_1'])=='' and trim($_REQUEST['rq_2'])=='')
        {
            
        }
        else
        {
            $filer1 .=" and (rq >= '" . $_REQUEST['rq_1']." 00:00:00' and rq <= '" . $_REQUEST['rq_2']." 23:59:59')";  
        }  
    }
...
function get_action_log_mx($ma,$sql)
{   
...
function update_action_log_mx($tb,$field,$u_id,$time_f)
{   
...
function insert_action_log_mx($tb,$field,$time_f)
{
    $array = explode(",",$field);
