...
if ($_REQUEST['m'] == 'default') {
    ...
    $smarty->assign('smtp_val', $get_smtp['smtp']);
    $smarty->assign('smtp', $get_smtp['list']);
...
if ($_REQUEST['m'] == 'setsys') {
    ...
    if (isset($_REQUEST['smtp1'])) {
        $smtp1 = $_REQUEST['smtp1'];
    }
    if (isset($_REQUEST['smtp2'])) {
        $smtp2 = $_REQUEST['smtp2'];
    }
    function set_smtp($val, $keyword, $wx_name, $wx_password)
    {
        $sql = "update set_sys set val='" . $val . "' where keyword='" . $keyword . "' ";
    ...
    set_smtp($smtp, "smtp", $smtp1, $smtp2);
