...
function download_goods_list($token, $res)
{
    $MENU_URL = "https://api.weixin.qq.com/merchant/get?access_token=" .
        $token;
    $ch = curl_init();
    curl_setopt($ch, CURLOPT_URL, $MENU_URL);
    curl_setopt($ch, CURLOPT_CUSTOMREQUEST, "POST");
...
function weixiaodian_order($obj,$ACC_TOKEN)
{
    $txt = "";
    ...
    for ($i = 0; $i < count($obj2); $i++) {
        ...
        if (empty($sel)) {
            ...
            $goods_list=array("product_id"=>"".$obj2[$i]['product_id']."");
            //print_r($goods_list);
            $goods_list = json_encode($goods_list);
            $goods_list = download_goods_list($ACC_TOKEN, $goods_list);
            
            $glist=$goods_list['product_info']['sku_list'];
       // print_r($glist);
            for($k=0;$k<count($glist);$k++)
            {
                if($glist[$k]['sku_id']==$obj2[$i]['product_sku'])
                {
                    $obj2[$i]['ori_price']=$glist[$k]['ori_price']/100;
                }
            }
            if($glist[0]['ori_price']=='')
            {
                $obj2[$i]['ori_price']=$glist['ori_price']/100;
            }
            else
            {
                $obj2[$i]['ori_price']=$glist[0]['ori_price']/100;
            }
            
            
            
            $obj2[$i]['ori_price_total']=$obj2[$i]['ori_price']*$obj2[$i]['product_count'];
            ...
            $sql = "insert into weixiaodian (
                                    ...
                                    last_update_2,ori_price,ori_price_total	
                ...
                "','" . $obj2[$i]['trans_id'] . "','" . $time2 . "','" . $obj2[$i]['ori_price'] . "','" . $obj2[$i]['ori_price_total'] . "')";
...
for ($i = 0; $i < count($status); $i++) {
    ...
    if (!empty($order_list['order_list'])) {
        ...
        $msg = weixiaodian_order($order_list,$ACC_TOKEN);
        $xx .= $msg;
    }
