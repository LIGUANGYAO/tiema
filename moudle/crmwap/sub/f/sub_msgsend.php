...
function get_msgsend_list($Nu, $ma, $sql)
{
    ...
    if (isset($_REQUEST['m_key'])) {
        ...
        $f2="";
        $f2.="msgsend_sn like '%" . trim($_REQUEST['m_key']) . "%' or ";
   
        $f2.="user_name like '%" . trim($_REQUEST['m_key']) . "%' or ";
    ...
    if(isset($_REQUEST['send_time_1']) and isset($_REQUEST['send_time_2']))
    {
        if(trim($_REQUEST['send_time_1'])=='' and trim($_REQUEST['send_time_2'])=='')
        {
            
        }
        else
        {
            $filer1 .=" and (send_time >= '" . $_REQUEST['send_time_1']."' and send_time <= '" . $_REQUEST['send_time_2']."')"; 
        }   
    }
    ...
    if(isset($_REQUEST['fasongleixing']))
    {
        if(trim($_REQUEST['fasongleixing'])=='')
        {
            
        }
        else
        {
            $filer1 .=" and fasongleixing = '" . trim($_REQUEST['fasongleixing'])."'";    
        }
    ...
    $filer = " where  kehu_id='".$_REQUEST['kehu_id']."' and user_name='".$us."'  $filer1 ";
    $action_list = array();
    ...
    $sql = $sql . $filer . " order by id desc ;";
    //" . $obj['limit_obj'] . "
    //echo $sql;
    //order by id  desc 
...
function get_msgsend_mx($ma,$sql)
{   
...
function update_msgsend_mx($tb,$field,$u_id,$time_f)
{   
...
function insert_msgsend_mx($tb,$field,$time_f)
{
    $array = explode(",",$field);
