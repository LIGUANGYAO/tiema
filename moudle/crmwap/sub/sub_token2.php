...
function token($Nu, $tb)
{
    $sql = "select id,token_sn,token_name,action,method,page,num,temporary_time,temporary_out,txtlog,exsql,tzsy,reformat,creatsub 
            from token";
    ...
    if (isset($_REQUEST['m_key'])) {
        $filer1 = " and ( token_sn like '%" . trim($_REQUEST['m_key']) .
            "%' or token_name like '%" . trim($_REQUEST['m_key']) . "%')";
    } else {
        $filer1 = '';
    }
    ...
    {
        $point="select * from token_mx where token_sn='".$res[$j]['token_sn']."' ";
       
        $res[$j]['token_mx']=$GLOBALS['db']->getAll($point);
    }
...
function i_token($tb, $field, $time_f)
{
...
function update_token_mx($tb,$field,$u_id,$time_f)
{   
...
{
  $sql="select bh from api_bhwh where cj_name='token'";
    $res = $GLOBALS['db']->getAll($sql);
        //$noNum=20;
...
{
  $sql="update api_bhwh set bh=bh+1 where cj_name='token'";
  $res = $GLOBALS['db']->query($sql);
}
...
function insert_token($tb,$field,$time_f)
{
    $array = explode(",",$field);
