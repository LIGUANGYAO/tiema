...
if ($_REQUEST['g'] == 'default') {
    ...
    function kehu_id_select($obj){
        $sql="select id,kehu_sn as sn,kehu_name as name,tel from kehu where tzsy=0 and 1=1 and id='".$obj."'";
    ...
    {
        $yinhang_id_select =kehu_id_select($list['items'][$j]['kehu_id']);
        $list['items'][$j]['kehu_id_bm']=$yinhang_id_select['name'];
        $list['items'][$j]['kehu_tel']=$yinhang_id_select['tel'];
    }
...
if ($_REQUEST['g'] == $g_e) {
    ...
    $smarty->assign('pinggu_mx', $pinggu_mx['items'][0]);
     ...
     function kehu_id_select($obj){
        $sql="select id,kehu_sn as sn,kehu_name as name,tel from kehu where tzsy=0  and id='".$obj."'";
        $res = $GLOBALS['db']->getRow($sql);
        return $res;
    }
 
    $kehu_id_select=kehu_id_select($pinggu_mx['items'][0]['kehu_id']);
    ...
    $gongsi_id_select=gongsi_id_select($pinggu_mx['items'][0]['gongsi_id']);
    $smarty->assign('gongsi_id_select', $gongsi_id_select);
    
    
     function yinhang_id_select($obj){
        $sql="select id,yinhang_sn as sn,yinhang_name as name from yinhang where tzsy=0 and id='".$obj."'";
        $res = $GLOBALS['db']->getRow($sql);
        return $res;
    }
 
    $yinhang_id_select=yinhang_id_select($pinggu_mx['items'][0]['yinhang_id']);
    ...
    $smarty->display('w/pinggu_mx.html');
}
