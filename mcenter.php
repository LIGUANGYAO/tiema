...
if ($_REQUEST['act'] == 'default') {
    ...
    if (empty($res[0]['wx_tel'])) {
        ...
        $get_fans = "select b.p_id,count(a.openid) as sl from cj_qrcode_stat a inner join tgpoint b on a.cj_sn=b.p_id where b.openid='" . $openid . "' group by b.p_id";
        $get_fans = $GLOBALS['db']->getRow($get_fans);
        $smarty->assign('fanssl', $get_fans['sl']);
        ...
        function fenc_5($p_id, $tp, $fenc)
        {
            ...
            for ($j = 0; $j < count($res); $j++) {
                ...
                */
                ...
                $res[$j]['fc'] = round($res[$j]['order_total_price'] *(($res[$j]['order_total_price']-$res[$j]['ori_price_total'] *$fenc / 100) /$res[$j]['order_total_price']-10/100));
                
                //echo $res[$j]['fc'];
                $res[$j]['djfc'] = round($res[$j]['order_total_price'] *(($res[$j]['order_total_price']-$res[$j]['ori_price_total'] *$fenc / 100) /$res[$j]['order_total_price']-10/100));
                //echo $res[$j]['fc']." ".$res[$j]['ori_price_total'] *$fenc / 100;
...
//                    2), 2) * $res[$j]['order_total_price'];
    ...
    function shop_to_sales4($p_id)
    {
        ...
        for ($i = 0; $i < count($res); $i++) {
            ...
            $res[$i]['sd_dgfc'] = fenc_5($res[$i]['sales_sn'], "sales", $res[$i]['dgpoint']);
 //print_r($res[$i]['sd_dgfc']);
        }
        ...
        for ($i = 0; $i < count($get_point); $i++) {
            if ($get_point[$i]['p_type'] == "1") {
                ...
                */
                ...
                $get_point[$i]['fc_list'] = fenc_4($res['sales_sn'], 'sales', $get_point[$i]['point']);
                //获取二维码地址
                $ticket = $res['ticket'];
                if (!empty($ticket)) {
...
if ($_REQUEST['act'] == 'set_or_status') {
    ...
    if (isset($_REQUEST['sn'])) {
        ...
        $sql = "update weixiaodian set or_status='" . $_REQUEST['or_status'] .
            "',queren_time='".$time."',bz='手工确认' where order_sn='" . $_REQUEST['sn'] . "'";
        $res = $GLOBALS['db']->query($sql);
        echo 1;
    } else {
        echo 2;
...
if ($_REQUEST['act'] == 'fxfc_w') {
    ...
    function getFilter($n,$y){
     ...
     switch($n){
         ...
         break;
         case 8://下月
             $strDate=date('Y-m-d',strtotime($y.' month'));
             $ret=getMonthRange($strDate);
         break;
    ...
    $yf=(int)trim($_REQUEST['yf']);
    $now_m=(int)date('m');
    
    if(isset($_REQUEST['year']))
    {
        $now_y=(int)trim($_REQUEST['year']);
    }
    else
    {
        $now_y=(int)date('Y');
    ...
    //Array ( [sdate] => 2015-07-1 00:00:00 [edate] => 2015-07-31 23:59:59 )
    //5获取当月开始日期和结束日期  6获取上月  7获取下月
    ...
    //判断当前月和年份已经最新
    if($now_m>=(int)date('m') and $now_y>=(int)date('Y'))
    {
        //echo $now_m;echo $now_y;echo (int)date('m');echo (int)date('Y');
        $smarty->assign('unxy', 0);
    }
    else
    {
        $smarty->assign('unxy', 1);
    }
    $m_array=array("by"=>$now_m,"xy"=>$xy,"sy"=>$sy,"year"=>$now_y,"syear"=>$now_y-1,"xyear"=>$now_y+1);
    
    $secan = 1;//如果是1的时候是大华版本
    if ($secan == 1) {

        function fenc_2($p_id, $tp, $fenc,$data_arr)
        {
            ...
            $sql_g = "select  a.ori_price_total,a.ori_price,a.or_status,a.product_price,a.order_total_price,a.order_status,case when a.order_status=2 then '待发货' when a.order_status=3 then '已发货' when a.order_status=5 then '已完成' when a.order_status=8 then '维权' end  as status_name,a.order_express_price,a.buyer_nick,a.buyer_openid,b.cj_sn,b.cj_type, from_unixtime(a.order_create_time) as order_create_time,a.order_sn,a.product_name,a.product_count,a.order_total_price  from weixiaodian a inner join cj_qrcode_stat b on a.buyer_openid=b.openid where cj_type='" .
                $tp . "' and cj_sn='" . $p_id .
                "' and from_unixtime(a.order_create_time) between '".$data_arr['sdate']."' and '".$data_arr['edate']."' group by b.openid,a.order_sn order by from_unixtime(a.order_create_time) desc   ";
            $res = $GLOBALS['db']->getAll($sql_g);
            //print_r($sql_g);
        ...
        function fenc_3($p_id, $tp, $fenc,$data_arr)
        {
        ...
        function fenc_4($p_id, $tp, $fenc,$data_arr)
        {
        ...
        //二级的间接分成
        function fenc_5($p_id, $tp, $fenc,$data_arr)
        {
            ...
            $res = $GLOBALS['db']->getAll($sql_g);
           // print_r($sql_g);exit;
if($oldopenid=='omQkMuEXDKDbU0oa8-ntqIy6Ggek')
                {
                   print_r($sql_g);
                }
            //设置分成
    ...
    function shop_to_sales4($p_id,$data_arr)
    {
        $t1 = date('Y-m-d H:i:s', time());
        ...
        for ($i = 0; $i < count($res); $i++) {
            ...
            $res[$i]['sd_dgfc'] = fenc_5($res[$i]['sales_sn'], "sales", $res[$i]['dgpoint'],$data_arr);
 //print_r($res[$i]['sd_dgfc']);
        }
    ...
    function tg_shop($p_id,$data_arr)
    {
        $t1 = date('Y-m-d H:i:s', time());
        ...
        for ($i = 0; $i < count($res); $i++) {
            ...
            $res[$i]['sdfc'] = fenc_3($res[$i]['shop_sn'], "shop", $res[$i]['sdpoint'],$data_arr);
    ...
    function tg_sales($p_id,$data_arr)
    {
        $t1 = date('Y-m-d H:i:s', time());
        ...
        for ($i = 0; $i < count($res); $i++) {
            ...
            $res[$i]['dgfc'] = fenc_3($res[$i]['sales_sn'], "sales", $res[$i]['dgpoint'],$data_arr);
...
//        }
    if (isset($_REQUEST['openid'])) {
        ...
        if($openid=='omQkMuEXDKDbU0oa8-ntqIy6Ggek')
        {
            $oldopenid='omQkMuEXDKDbU0oa8-ntqIy6Ggek';
            $openid='omQkMuG99BXz04f6CL1-Y616tcvQ';
        }
        ...
        for ($i = 0; $i < count($get_point); $i++) {
            if ($get_point[$i]['p_type'] == "1") {
                ...
                */
                ...
                //开始计算渠道下属店铺的分成比例
                $get_point[$i]['fc_list'] = fenc_2($res['qudao_sn'], 'qudao', $get_point[$i]['point'],$data_arr);
                //print_r($get_point[$i]['fc_list'] );


                $get_point[$i]['shop_fenc'] = tg_shop($res['qudao_sn'],$data_arr);
                //--end结束下属商店结算

                //开始计算渠道下属店铺的分成比例


                $get_point[$i]['sales_fenc'] = tg_sales($res['qudao_sn'],$data_arr);
                //--end结束下属商店结算
                ...
                */
                $get_point[$i]['fc_list'] = fenc_2($res['shop_sn'], 'shop', $get_point[$i]['point'],$data_arr);
                ...
                $get_point[$i]['shop_sales_fenc'] = shop_to_sales4($res['shop_sn'],$data_arr);
                //--end结束下属商店结算
...
//                }
                ...
                $get_point[$i]['fc_list'] = fenc_4($res['sales_sn'], 'sales', $get_point[$i]['point'],$data_arr);
                //获取二维码地址
                $ticket = $res['ticket'];
                if (!empty($ticket)) {
        ...
        $smarty->assign('yf', $m_array);
        $smarty->display('w/mcenter.html');
