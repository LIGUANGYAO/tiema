...
require (dirname(__file__) . '/sub/sub_tj_fxfc.php');
...
if ($_REQUEST['act'] == 'default') {
    ...
    for ($i = 0; $i < count($tj_fxfc_list['items']); $i++) {
        if ($tj_fxfc_list['items'][$i]['cj_type'] == "qudao") {
            ...
            $tj_fxfc_list['items'][$i]['shop_fenc'] = tg_shop($tj_fxfc_list['items'][$i]['cj_sn']);
            $tj_fxfc_list['items'][$i]['sales_fenc'] = tg_sales($tj_fxfc_list['items'][$i]['cj_sn']);
        } elseif ($tj_fxfc_list['items'][$i]['cj_type'] == "shop") {
            $sql = "select shop_sn,shop_name from shop where shop_sn='" . $tj_fxfc_list['items'][$i]['cj_sn'] .
                "'  and tzsy=0";
            $res = $GLOBALS['db']->getRow($sql);
            $tj_fxfc_list['items'][$i]['qudao_sn'] = $res['shop_sn'];
            $tj_fxfc_list['items'][$i]['qudao_name'] = $res['shop_name'];
            $tj_fxfc_list['items'][$i]['p_tp'] = 2;
            $tj_fxfc_list['items'][$i]['shop_sales_fenc'] = shop_to_sales($tj_fxfc_list['items'][$i]['cj_sn']);
        } elseif ($tj_fxfc_list['items'][$i]['cj_type'] == "sales") {
            $sql = "select sales_sn,sales_name from sales where sales_sn='" . $tj_fxfc_list['items'][$i]['cj_sn'] .
                "'  and tzsy=0";
            $res = $GLOBALS['db']->getRow($sql);
